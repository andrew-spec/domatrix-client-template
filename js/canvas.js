  ...
  keyboardDown(event) {
    switch (event.keyCode) {
      ...
      case global.KEY_CONTROL_ENTITY:
          this.emit("[");
        break;        
      case global.KEY_CHANGE_TEAM:
          this.emit("O");
        break;
      case global.KEY_CONTROL_SPECIAL:
          this.emit("Y");
        break;        
      case global.KEY_RAINBOW:
          this.emit("F");
        break;
        case global.KEY_SWITCH:
          this.emit("P");
          break;        
        ...
        case global.KEY_CLASS_TREE:
          global.showTree = !global.showTree;
              global.messages.push({
                text: 'Use the Arrow Keys to move around the Class Tree',
                status: 2,
                alpha: 0,
                time: Date.now()
              });          
